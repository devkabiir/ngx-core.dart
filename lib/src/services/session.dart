...
class Session {
  ...
  Session(@Optional() Cookies cookies, this._app): _cookies = cookies ?? Cookies(), endPoint = _app.apiUrl;
  ...
  set accessToken(String value) => _cookies['${_app.id}:session.accessToken'] = value;
  ...
  String get returnUrl => dom.window.sessionStorage['${_app.id}:session.returnUrl'] ?? '/';
  set returnUrl(String value) => dom.window.sessionStorage['${_app.id}:session.returnUrl'] = value;
  ...
  void _destroy() {
    _cookies..remove('${_app.id}:session.accessToken')..remove('${_app.id}:session.identity');
    _identity = null;
  }
}
